...
class SendConfig(PluginConfigBase):
    ...
    enable_adapter_open_tool: bool = Field(
        default=True,
        description="加载时尝试启用 Napcat 适配器的 open_private_chat 工具，便于首次联系时建立 15 分钟临时放行窗口",
    )
...
class _RelayTask:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "_RelayTask":
        ...
        return cls(
            ...
            buffer=[str(item) for item in (data.get("buffer") or [])],
        )
...
class PrivateChatToolPlusPlugin(MaiBotPlugin):
    ...
    async def tool_send_private_chat(
        ...
        **kwargs: Any,
        ...
        # 5. 通过宿主统一发送管线发送（正常入库、进入对方私聊流上下文）
        try:
    ...
    async def _try_enable_adapter_tool(self) -> None:
        """尝试启用适配器的 open_private_chat 工具（可配置关闭）。

        该工具发送首条消息时会授予 15 分钟的私聊名单临时放行，
        便于「首次联系 + 等待回复」的场景在默认配置下也能收到对方回复。
        """

        if not self.config.send.enable_adapter_open_tool:
            return
        try:
            ...
            if isinstance(result, dict) and result.get("success") is False:
                ...
                self.ctx.logger.info("已启用适配器 open_private_chat 工具（首次联系时可建立临时私聊放行）")
        except Exception as exc:
